...
st.set_page_config(page_title='Relatório de 26/11/2024', layout='wide', page_icon='📊')

#################### Título da página do Streamlit ####################
st.markdown('# Relatório de Respostas - 26/11/2024')

#################### Leitura e Tratamento dos Dados ####################
# Ler os dados originais
grafico_aposentados_totais_df = pd.read_csv('grafico_aposentados_totais_26_11.csv')

# Filtrar as respostas completas
grafico_aposentados_completas_df = pd.read_csv('grafico_aposentados_completas_26_11.csv')

# Filtrar as respostas incompletas
grafico_aposentados_incompletas_df = pd.read_csv('grafico_aposentados_incompletas_26_11.csv')
...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 289)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 145)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 144)
                        
                # Criar a métrica do total de avaliações feitas
                with col_4:
                        col_1.metric('Instituições de Ensino Citadas:', 45)

        # Criar as métricas semenais
        with st.container():
                st.title('Crescimento Semanal (15/11/2024 a 26/11/2024)')
                col_1, col_2, col_3, col_4 = st.columns(4, gap='large')
                
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 12)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 5)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 7)
                        
                # Criar a métrica do total de avaliações feitas
                with col_4:
                        col_1.metric('Instituições de Ensino Citadas:', 0)
...
with tab_2:
    ...
    fig = px.bar(grafico_aposentados_totais_df, x='uf_instituicao', y='contagem', color='instituicao',
                ...
                title='Respostas Totais por UF e Instituição de Ensino (26/11/2024) - Aposentados Não-ativos')
    ...
    tabela_aposentados_totais_df = pd.read_csv('tabela_aposentados_totais_26_11.csv')
...
with tab_3:
    ...
    fig = px.bar(grafico_aposentados_completas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas completas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Completas por UF e Instituição de Ensino (26/11/2024) - Aposentados Não-ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_aposentados_completas_df = pd.read_csv('tabela_aposentados_completas_26_11.csv')
...
with tab_4:
    ...
    fig = px.bar(grafico_aposentados_incompletas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas incompletas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Incompletas por UF e Instituição de Ensino (26/11/2024) - Aposentados Não-ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_aposentados_incompletas_df = pd.read_csv('tabela_aposentados_incompletas_26_11.csv')
