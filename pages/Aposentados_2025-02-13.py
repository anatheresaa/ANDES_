...
st.set_page_config(page_title='Relatório de 13/02/2025', layout='wide', page_icon='📊')

#################### Título da página do Streamlit ####################
st.markdown('# Relatório de Respostas - 13/02/2025')

#################### Leitura e Tratamento dos Dados ####################
# Ler os dados originais
grafico_aposentados_totais_df = pd.read_csv('tabela_aposentados_totais_13_02.csv')
...
grafico_aposentados_completos_df = pd.read_csv('tabela_aposentados_completos_13_02.csv')
...
grafico_aposentados_incompletos_df = pd.read_csv('tabela_aposentados_incompletos_13_02.csv')
...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 555)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 281)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletos:', 274)
        ...
        with st.container():
                st.title('Crescimento Semanal (07/02/2025 a 13/02/2025)')
                col_1, col_2, col_3, col_4 = st.columns(4, gap='large')
                
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 4)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 2)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletos:', 2)
...
with tab_2:
    ...
    fig = px.bar(grafico_aposentados_totais_df, x='UF', y='Contagem', color='Instituição de ensino',
                ...
                title='Respostas Totais por UF e Instituição de Ensino (13/02/2025) - aposentados')
    ...
    tabela_aposentados_totais_df = pd.read_csv('tabela_aposentados_totais_13_02.csv')
...
with tab_3:
    ...
    fig = px.bar(grafico_aposentados_completos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas completas',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas completos por UF e Instituição de Ensino (13/02/2025) - aposentados')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_aposentados_completos_df = pd.read_csv('tabela_aposentados_completos_13_02.csv')
...
with tab_4:
    ...
    fig = px.bar(grafico_aposentados_incompletos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas incompletas',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas Incompletos por UF e Instituição de Ensino (13/02/2025) - aposentados')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_aposentados_incompletos_df = pd.read_csv('tabela_aposentados_incompletos_13_02.csv')
