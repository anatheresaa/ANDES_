...
grafico_ativos_totais_df = pd.read_csv('grafico_ativos_totais_08_11.csv')

# Filtrar as respostas completas
grafico_ativos_completas_df = pd.read_csv('grafico_ativos_completas_08_11.csv')

# Filtrar as respostas incompletas
grafico_ativos_incompletas_df = pd.read_csv('grafico_ativos_incompletas_08_11.csv')
...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 2372)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 1510)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 862)
                        
                # Criar a métrica do total de avaliações feitas
                with col_4:
                        col_1.metric('Instituições de Ensino Citadas:', 119)
        ...
        with st.container():
                ...
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 278)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 176)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 102)
...
with tab_2:
    ...
    fig = px.bar(grafico_ativos_totais_df, x='uf_instituicao', y='contagem', color='instituicao',
                ...
                title='Respostas Totais por UF e Instituição de Ensino (08/11/2024) - Servidores e Aposentados em Atividade')
    ...
    tabela_ativos_totais_df = pd.read_csv('tabela_ativos_totais_08_11.csv')
...
with tab_3:
    ...
    fig = px.bar(grafico_ativos_completas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas completas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Completas por UF e Instituição de Ensino (08/11/2024) - Servidores e Aposentados em Atividade')
    fig.update_yaxes(tick0=0, dtick=20)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_completas_df = pd.read_csv('tabela_ativos_completas_08_11.csv')
...
with tab_4:
    ...
    fig = px.bar(grafico_ativos_incompletas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas incompletas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Incompletas por UF e Instituição de Ensino (08/11/2024) - Servidores e Aposentados em Atividade')
    fig.update_yaxes(tick0=0, dtick=20)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_incompletas_df = pd.read_csv('tabela_ativos_incompletas_08_11.csv')
