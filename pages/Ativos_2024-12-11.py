...
grafico_ativos_totais_df = pd.read_csv('grafico_ativos_totais_11_12.csv')

# Filtrar as respostas completas
grafico_ativos_completas_df = pd.read_csv('grafico_ativos_completas_11_12.csv')

# Filtrar as respostas incompletas
grafico_ativos_incompletas_df = pd.read_csv('grafico_ativos_incompletas_11_12.csv')
...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 3642)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 2280)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 1362)
                        
                # Criar a métrica do total de avaliações feitas
                with col_4:
                        col_1.metric('Instituições de Ensino Citadas:', 127)
        ...
        with st.container():
                ...
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 394)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas Completas:', 212)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletas:', 182)
...
with tab_2:
    ...
    fig = px.bar(grafico_ativos_totais_df, x='uf_instituicao', y='contagem', color='instituicao',
                ...
                title='Respostas Totais por UF e Instituição de Ensino (11/12/2024) - Servidores e Aposentados em Atividade')
...
with tab_3:
    ...
    fig = px.bar(grafico_ativos_completas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas completas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Completas por UF e Instituição de Ensino (11/12/2024) - Servidores e Aposentados em Atividade')
    fig.update_yaxes(tick0=0, dtick=20)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_completas_df = pd.read_csv('tabela_ativos_completas_11_12.csv')
...
with tab_4:
    ...
    fig = px.bar(grafico_ativos_incompletas_df, x='uf_instituicao', y='contagem', color='instituicao',
                labels = {'uf_instituicao':'UF', 'contagem':'Respostas incompletas',
                        'instituicao':'Instituição de ensino'},
                title='Respostas Incompletas por UF e Instituição de Ensino (11/12/2024) - Servidores e Aposentados em Atividade')
    fig.update_yaxes(tick0=0, dtick=20)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_incompletas_df = pd.read_csv('tabela_ativos_incompletas_11_12.csv')
