...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 4060)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 2567)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletos:', 1493)
                        
                # Criar a métrica do total de avaliações feitas
                with col_4:
                        col_1.metric('Instituições de Ensino Citadas:', 133)
        ...
        with st.container():
                ...
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 418)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 287)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletos:', 131)
...
with tab_3:
    ...
    fig = px.bar(grafico_ativos_completos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas completos',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas completos por UF e Instituição de Ensino (16/12/2024) - Ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
...
with tab_4:
    ...
    fig = px.bar(grafico_ativos_incompletos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas incompletos',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas Incompletos por UF e Instituição de Ensino (16/12/2024) - Ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
