...
grafico_ativos_totais_df = pd.read_csv('tabela_ativos_totais_13_02.csv')
...
grafico_ativos_completos_df = pd.read_csv('tabela_ativos_completos_13_02.csv')
...
grafico_ativos_incompletos_df = pd.read_csv('tabela_ativos_incompletos_13_02.csv')
...
with tab_1:
        ...
        with st.container():
                ...
                with col_1:
                        col_1.metric('Respostas Totais:', 5007)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 3188)
                        
                # Criar a métrica de cidades registrados
                with col_3:
                        col_1.metric('Respostas Incompletos:', 1819)
        ...
        with st.container():
                st.title('Crescimento Semanal (13/02/2025 a 13/02/2025)')
                col_1, col_2, col_3, col_4 = st.columns(4, gap='large')
                
                # Criar a métrica de restaurantes registrados
                with col_1:
                        col_1.metric('Respostas Totais:', 46)
                        
                # Criar a métrica de países registrados
                with col_2:
                        col_1.metric('Respostas completos:', 33)
...
with tab_2:
    ...
    fig = px.bar(grafico_ativos_totais_df, x='UF', y='Contagem', color='Instituição de ensino',
                ...
                title='Respostas Totais por UF e Instituição de Ensino (13/02/2025) - Ativos')
    ...
    tabela_ativos_totais_df = pd.read_csv('tabela_ativos_totais_13_02.csv')
...
with tab_3:
    ...
    fig = px.bar(grafico_ativos_completos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas completos',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas completos por UF e Instituição de Ensino (13/02/2025) - Ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_completos_df = pd.read_csv('tabela_ativos_completos_13_02.csv')
...
with tab_4:
    ...
    fig = px.bar(grafico_ativos_incompletos_df, x='UF', y='Contagem', color='Instituição de ensino',
                labels = {'UF':'UF', 'Contagem':'Respostas incompletos',
                        'Instituição de ensino':'Instituição de ensino'},
                title='Respostas Incompletos por UF e Instituição de Ensino (13/02/2025) - Ativos')
    fig.update_yaxes(tick0=0, dtick=10)
    fig.update_layout(showlegend=False)
    st.plotly_chart(fig, use_container_width=True)
    ...
    tabela_ativos_incompletos_df = pd.read_csv('tabela_ativos_incompletos_13_02.csv')
